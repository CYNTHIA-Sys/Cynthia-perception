...
namespace cute
{
	...
	// lists directory catalog with given directory
	static std::vector< std::string > directory_list ( const char* dir ,
	                                                   int flg )
	{
		// MAX_PATH  = 255
		DIR* dfd = opendir( dir );
		...
		while ( ( dp = readdir( dfd ) ) != NULL )
		{
			if ( strcmp( dp->d_name , "." ) == 0 )
				continue;
			if ( flg == FILES_AND_DIRECTORIES )
				dlist.push_back( std::string( dp->d_name ) );
			else
			{
				char path[1000];
				sprintf( path , "%s/%s" , dir , dp->d_name );
				int is = is_directory( path );
				if ( flg == FILES && ( !is ) )
					dlist.push_back( std::string( dp->d_name ) );
				else if ( flg == DIRECTORIES && is )
					dlist.push_back( std::string( dp->d_name ) );
			}
		}
		closedir( dfd );
